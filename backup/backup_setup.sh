...
source ~/.bashrc
source ~/.profile
(crontab -l ; echo "27 6 * * * . ~/.profile; cd ~/ECE458-Group-Project/backup && bash backup.sh daily >> daily_backup.log 2>&1") 2>&1 | sed "s/no crontab for $(whoami)//"  | sort | uniq | crontab
(crontab -l ; echo "0 0 * * 0 . ~/.profile; cd ~/ECE458-Group-Project/backup && bash backup.sh weekly >> weekly_backup.log 2>&1") 2>&1 | sed "s/no crontab for $(whoami)//"  | sort | uniq | crontab
(crontab -l ; echo "0 0 1 * * . ~/.profile; cd ~/ECE458-Group-Project/backup && bash backup.sh monthly >> monthly_backup.log 2>&1") 2>&1 | sed "s/no crontab for $(whoami)//"  | sort | uniq | crontab
